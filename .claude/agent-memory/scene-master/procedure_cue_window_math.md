...
description: cue 창·split·delayMs·아웃트로 여유를 마스터가 직접 검산하는 식 — 분모는 rel, 어절은 dur, 카드는 TOTAL-2600
metadata:
  type: reference
---
...
- `:163` `dur = durOf(si,li,say) + (l.pauseAfter ?? 0)` — 자막 레코드의 dur 은 **wav + pause**
- `:167` 샷 dur 에 `SHOT_TAIL * 1000`(0.35초) 이 **샷마다** 더해진다
- `:169` `rel = ls.map(l => ({ t: …, dur: l.dur/1000 }))` — 그래서 `rel.dur` 도 **wav + pause**
- `:292` `cue(i, lead, span)` = `a = l.t − lead`, `b = l.t + l.dur*span`, `(ts − a)/(b − a)` 로 0~1 클램프
- `:181,259` `OUTRO_MS = 2600` · 아웃트로 카드는 `t ≥ TOTAL − 2600`
...
**③ 아웃트로 여유** — 마지막 샷에서만 쓴다.
`카드(샷 기준) = (마지막 두 자막 rel 합 + 350) − 2600`.
