...
- [가림은 스틸 한 장이 아니라 수식으로 잰다](review_method_occlusion_measure.md) — 둥근 캡 반경 + 불투명(α>0.7) 겹침만 세기. 🔴 **색 군집 세기로 「없다」를 증명하지 마라** — 가려지면 거짓 0(lf01 3차)
...
- 🔴 [check 전에 build/timed.json 샷 수를 대조하라](review_method_stale_timed_json.md) — 옛 더미 타임라인이 길이·말속도·자막2초 게이트를 대신 통과시킨다(lf01 실측: 34샷을 「10.6초」로 통과). 꼬리 ÷0.35 = 샷 수

## 회차 기록
- [회차별 판정과 반려 횟수](project_review_log.md) — "반려 3회면 사람에게" 규칙을 세기 위한 표. ep00s·ep01s 각 1차 반려(2026-07-29), ep02s 반려 2회 후 3차 통과, 재작업 트랙은 1차 반려 → 2차 통과(2026-07-30). 🔴 **사건 단위 재분할 여섯 편(2026-08-04)** — 2차까지 돌려 PR #10 으로 main 머지. **롱폼 lf01·lf02 둘 다 반려 2회 후 3차 통과** (lf02 3차 = 일곱 자리 전부 해소 + 🔴 렌더 전 차단 2건 인계). 파일 끝 절에 트랙 전체 요약
