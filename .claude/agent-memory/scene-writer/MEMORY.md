...
- [효과음 delayMs 는 draw 를 풀어서 낸다 · gain 상한은 kind+옵션이 정한다](project_sfx_timing.md) — 실측 나오면 재계산 필수(같은 회차 안에서도 오차 3배). 🔴 **돌려막기는 형제 편의 kind 「쌍」으로도 난다** · 🔴 **`why` 에 「>0.5 임계」 쓰지 마라 — 소리는 리빌 시작에 놓인다**
...
- [캔버스 글자는 폭을 재서 안쪽에 가둔다](feedback_canvas_edge_text.md) — 폭 제한은 박스가 아니라 글자에. 아래 가장자리는 **한 프레임만 스쳐도 red**이고 보고값은 거리가 아니라 **픽셀 개수**(÷dpr = 범인의 CSS 폭). 🔴 **움직이는 요소는 「가장 밖으로 나가는 순간」으로**. 🔴 **겹침은 좌표가 아니라 부등식으로 막는다** · wide 는 엔진 HUD 가 캔버스 y≤21 을 덮는다
- [화면 요약은 수·배분·종류를 다 셀 수 없으면 말하지 않는다](feedback_frame_summary_scope.md) — lf02 S1 이 세 번 반려됐다. 「다섯 명」(수) → 「저마다」(배분) → 「계획」(종류)이 차례로 걸렸다. 종류도 셈이다
