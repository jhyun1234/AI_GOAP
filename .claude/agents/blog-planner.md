...
3. **git 커밋 교차 검증**: 같은 기간에 `git log <last_hash>..HEAD --stat` 로 실제 커밋을
   ...
   명시한다(로그 작성 누락 감시).
3-B. 🔴 **화면 줄 수집 (2026-08-13 신설 — 3D 영상 트랙의 재료)**: 같은 커밋 범위에
   `git log <last_hash>..HEAD --format='%h %b' | grep -n '화면:'` 으로 **`화면:` 줄을
   전부 뽑는다.** 이 줄은 커밋 규약상 **사람이 주어이고 동사로 끝나는 현재형 한 줄**이며
   (`Docs/CLAUDE.md` "자동 커밋 + 개발 일지"), 브리프 `screen_verbs:` 에 해시와 함께
   그대로 싣는다. `화면: 없음`인 커밋은 싣지 않는다.
   - 커밋에 `화면:` 이 하나도 없으면 세션 로그 "무엇을 했나"에서 **화면에서 벌어진 일**만
     골라 같은 문법(사람 주어 + 현재형 동사)으로 옮겨 적고, 출처를 세션 로그로 표시한다.
     🔴 **없는 화면을 지어내지 않는다** — 도구·문서 회차는 `screen_verbs: []` 가 정답이다.
   - 왜: 이 목록이 작성팀의 "장면으로 보여주기"와, 그 글을 받는 쇼츠·롱폼 3D 대본의
     **유일한 몸짓 재료**다. 3D 는 사람이 하는 일로만 말할 수 있다(`Docs/3D_대본_문법.md`).
...
deep_dive_posts:
  - tag: <#태그명>
    ...
    promise: <제목이 걸어야 할 약속(이 회차의 사건) 한 줄>   # 2026-08-08 신설 — 검수 9번 제목 이행 검사의 대조 기준
    payoff_evidence: <그 약속이 실제로 이행됐다는 근거 — 세션로그/커밋 참조>
    reversal: <믿었던 것 → 실제로 벌어진 것. 소재에 반전이 없으면 "없음">
    screen_verbs:                  # 2026-08-13 신설 — 화면에서 사람이 하는 일. 없으면 빈 목록
      - "<사람 주어 + 현재형 동사 한 줄>" — <출처: 커밋 <hash> 화면: 줄 / 세션로그 경로>
    quotes:                        # 작성팀이 따옴표 인용할 수 있는 유일한 원천 (없으면 빈 목록)
      - "<원문 그대로>" — <출처: 세션로그 경로 / Assets 파일:행번호 (verify_at 기준)>
...
- 🎁 **사건 우선 수집 (2026-08-08 신설 — 재미는 포집이지 생성이 아니다)**: 세션 로그의
  **"뒤집힌 것"·"육성 인용"** 칸을 브리프의 `reversal`·`quotes`로 옮기는 것이 소재 선정의
  일부다. 재미의 정체는 쇼츠 실측이 답했다 — **약속과 이행의 구조**. 그래서 `promise`
  (제목이 걸 사건)와 `payoff_evidence`(이행 근거)를 반드시 채운다.
  - 사용자 판정 육성("동선이 아주 잘 보여" 류)은 **세션 로그·memory에 적힌 그대로만**
    싣는다 — 기억으로 재구성하거나 다듬지 않는다.
  - 🔴 **반전을 지어내 채우지 않는다.** 로그에 뒤집힘이 없으면 `reversal: 없음`이 정답이다.
    없음인 회차는 작성팀이 종전 문제→해결 뼈대로 쓴다 — 그것도 정상 회차다.
