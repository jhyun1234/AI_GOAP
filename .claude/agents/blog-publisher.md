...
1. 기획팀이 이번 회차에 사용한 `selected_commits` 중 가장 최신 커밋 해시를
   **`tools/blog-automation/state/blog_last_published_commit.md`**(리포 상태 파일)에 기록한다.
   ...
   🔴 **이력 이동 시 ACTIVE 마커 무효화 (2026-08-08 신설)**: 소비된 블록을 이력 섹션으로
   내릴 때 "지정 당시 원문"의 `**STATUS: ACTIVE (...)**` 줄은 반드시
   `**STATUS: 이력(당시 ACTIVE — <원래 괄호>, YYYY-MM-DD 마커 무효화)**` 로 바꿔 보존한다.
   살아있는 `**STATUS: ACTIVE` 줄은 파일 전체에서 **최상단 1개뿐**이어야 한다 —
   selftest 4번이 이걸 세며, 원문을 그대로 두면 사이클마다 오탐 마커가 하나씩 쌓인다.
