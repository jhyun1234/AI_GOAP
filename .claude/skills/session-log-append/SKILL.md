...
description: devlog/sessions/YYYY-MM-DD.md에 세션 로그를 수동으로 append할 때 사용.
  기본 흐름은 PostToolUse 훅(tools/devlog/append-session-log.sh)이 처리하되 훅은 커밋에서
  뽑을 수 있는 칸(태그·무엇을·왜=커밋 본문·변경 파일)까지만 쓴다 — 이 스킬은 훅 실패 복구,
  소급 정리, 그리고 훅이 알 수 없는 **막혔던 문제 / 다음에 할 일** 보강에 발동한다.
...
- **왜 이렇게 했나:** 훅이 커밋 본문(`%b`)을 그대로 싣는다 — 수동 작성 시에도 같은 자리에.
  **커밋 본문에 "왜"를 쓰는 것이 곧 이 칸을 쓰는 것이다.**
- 🔴 **막혔던 문제 / 해결 방법:** 훅이 알 수 없는 칸(대화에만 있음). 해당 없으면
  "해당 없음"이라고 쓴다 — **빈 칸으로 두지 않는다.**
- 🔴 **다음에 할 일:** 위와 같음. 다음 세션이 이어받을 지점 1줄.
