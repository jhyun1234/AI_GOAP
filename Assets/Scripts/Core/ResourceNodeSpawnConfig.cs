...
namespace AIVillage.Core
{
    ...
    public class ResourceTypeSpawnData
    {
        ...
        [Tooltip("이 자원이 **설 수 있는** 지형. 비우면 아무 지형이나 (舊 동작 · 중립). " +
                 ...
                 "⚠️ 여기를 채우면 그 지형이 없는 판에서는 개수가 줄어든다 (경고 로그로 드러난다).")]
        public AIVillage.M0.TerrainTypeSO[] allowedTerrain;

        [Tooltip("이 지형에 **인접**하면 된다 (물가 = 물에 붙은 땅). 8방향. 비우면 조건 없음.\n" +
                 "🔴 물 '위'가 아니라 물에 '붙은 땅'이다 — 물은 통행 불가라 서면 아무도 못 닿는다.\n" +
                 "⚠️ allowedTerrain 과 둘 다 걸면 교집합이다 (평지이면서 물에 붙은 칸).")]
        public AIVillage.M0.TerrainTypeSO[] adjacentTerrain;
