...
namespace AIVillage.Core
{
    ...
    public sealed class VillageAdvisor : MonoBehaviour
    {
        ...
        private void TryEnqueue(string buildingId, AuthoritativeWorldState ws)
        {
            ...
            // ─────────────────────────────────────────────────────────────
            // 이미 대기열에 있는 건물은 조용히 스킵 — 5초 주기 재평가마다 EnqueueBuilding이
            // 거부 로그를 남기던 스팸을 차단한다. WorldState.BuildingQueued도 이미 true이므로 갱신 불필요.
            // ─────────────────────────────────────────────────────────────
            if (BuildingQueue.Instance.IsBuildingPending(buildingId))
            {
                return;
            }
