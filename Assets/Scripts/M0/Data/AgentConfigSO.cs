...
namespace AIVillage.M0
{
    ...
    public sealed class AgentConfigSO : ScriptableObject
    {
        ...
        // (M19-W5: 화폐 대사 3종 — 첫 임금·중과세 불평·물가 거부 — 은 화폐와 함께 철거)
        ...
        [Tooltip("당한 자리 둘레 이 반경(맨해튼)이 '위험했던 곳'이다. 정확한 좌표만 기억하면 " +
                 "한 칸 옆으로 비켜서 똑같이 당한다.\n" +
                 "🔴 W8 후속 실측(2026-08-11)으로 8→5 하향: 8이면 자원 클러스터(퍼짐 3)를 통째로 " +
                 "덮어 '옆 나무로 피하기'가 구조적으로 불가능했다 — 회피가 화면에 안 나온 원인 절반. " +
                 "5면 큰 클러스터의 반대편 가장자리는 벌점 밖이다.")]
        [Min(1)] public int DangerMemoryRadiusTiles = 5;

        [Tooltip("피격 1회가 남기는 기억 강도 (W8 후속 2026-08-11 — 舊 코드 상수 1을 에셋으로 승격 " +
                 "+ 3으로 상향). 회피 발현 조건 = 강도 × GatherDangerWeight(12) > 클러스터 간격 " +
                 "(배포 최대 30) — 1×12=12로는 어느 자원도 옆 클러스터로 못 돌아서서, 다섯 대를 " +
                 "물려도 같은 숲으로 갔다 (실측). 3×12=36 > 30: 한 번 물리면 발길을 돌린다. " +
                 "게이트 M26B_T6c가 이 부등식을 배포 값으로 감시한다.")]
        [Min(0f)] public float DangerMemoryPerHit = 3f;
        ...
        [Tooltip("간호받는 중(IsTended)인 부상자가 보류·정지하는 goal 우선순위 상한 (대상 협조, " +
                 "M26-2차 W7R — 사용자 결정 '치료받을 때는 가만히'). 이 값 이하는 치료가 끝날 때까지 " +
                 "미루고, 이동 중이면 멈춰 선다. 제안치 100 = 허기(100)·피로(90)·간식(30)까지 잡되 " +
                 "피신(105)만 통과 — 위협 앞에서 붙잡아 두면 치료가 사형 명령이 된다. " +
                 "0 이하 = 보류 없음 (중립 = 舊 동작).")]
        public int TendHoldMaxPriority = 100;
