...
namespace AIVillage.M0
{
    ...
    public sealed class GoalSO : ScriptableObject
    {
        ...
        [Tooltip("성격 서술의 **행동** 한 줄 (M25-1차) — 예: \"다친 사람을 보면 먼저 간다\".\n" +
                 "🔑 행동만 적는다. 사람됨은 축 문장이 말하므로, 여기에 성격을 적으면 같은 말을 " +
                 "두 번 하게 된다.\n" +
                 "비면 이 goal은 서술에 안 나온다 (중립 폴백 — 전 goal에 적을 필요가 없다). " +
                 "🔴 판정에는 쓰이지 않는다 (ADR-M25-6 — 표현 전용).")]
        ...
        [Tooltip("설정 시 플래너를 생략하고 이 풀에서 랜덤 1개를 즉시 실행한다 (M1-A).\n" +
                 ...
                 "     (교전·망루 — 끝이 위협/서비스의 상태라서 슬롯 조건으로 못 쓴다)\n" +
                 "  ⓑ 풀 안의 어느 것을 골라도 같은 뜻이다 — 플래너가 고를 이유가 없다\n" +
                 "  ⓒ 전역 스톡을 건드리지 않는다 — 플래너를 우회하므로 전제·효과 검증이 없다\n" +
                 "셋 중 하나라도 아니면 플래너에 맡긴다.\n" +
                 "현재 사용: 여가·일과 2종·**교전 1종(M21-W4)**·**망루 탑승(M22-3차 W3)**.\n" +
                 "  (2026-08-11 개정: 완공보고를 목록에서 제거 — 보고 심부름 사슬이 조각 Y '마주치면\n" +
                 "  정산' 전환의 후속 정리로 삭제됐다. ②종류의 원조 사례였으나 교전·망루가 대를 잇는다.)\n" +
                 "  탑승이 ⓐ에 드는 이유 = 교전과 같은 두 번째 종류: 끝은 '감지 해소'(ThreatService\n" +
                 "  상태를 러너가 판정)이고 슬롯에 적을 수 없다. ⓑ 풀 1개 ⓒ 요격 드랍은 CombatService 몫.\n" +
                 "  교전이 ⓐ에 드는 이유 = 두 번째 종류('끝이 슬롯 밖'): 교전의 끝은 '위협이 죽거나\n" +
                 "  물러남'이고 그건 ThreatAgent의 상태다 — ThreatNear는 파생 슬롯이라 '내가 이겼는가'를\n" +
                 "  적을 수 없다. 덤으로 **필요**이기도 하다: 도망 액션과 효과(ThreatNear→0)가 같아서\n" +
                 "  플래너에 맡기면 '싸우라'는 goal이 도망을 고를 수 있다.\n" +
        ...
        [Tooltip("**촌장 명령으로 주입됐을 때만** TriggerConditions를 우회한다 (M21-W4 재검사 B1). " +
                 "자율 선택에는 아무 영향이 없다 — 트리거는 그대로 지켜진다 (중립 불변식).\n" +
                 "왜 필요한가: 교전 goal의 트리거는 `ThreatNear==1`(근접)인데, 밭을 치는 위협에서 " +
                 "멀리 있는 주민은 ①트리거 미발동이라 자율 참전이 안 되고 ②명령해도 후보에서 " +
                 "탈락해 **명령 자체가 무효**가 된다. W8의 '⚔ 습격 중' 상시 표시가 개입을 권하는데 " +
                 "수단이 없는 공수표가 되므로, 원정을 명령으로만 연다.\n" +
                 "⚠️ 자율에 열면 안 된다 (명세 ⚠️ 오해 위험 ④): 전 맵 주민이 밭의 위협 한 마리로 " +
                 "몰려가는 과잉 반응이 된다. **원정은 플레이어 개입의 값**이다.\n" +
                 "⚠️ 달성 검사는 여기서 다루지 않는다 — 이 플래그를 쓰는 goal은 DirectActionPool " +
                 "goal이라 GoalConditions가 비어 있고, 애초에 '이미 달성' 스킵이 없다.")]
        public bool OrderedIgnoresTrigger;
