...
namespace AIVillage.M0
{
    ...
    ///
    /// ⚠️ 효율 배율의 사정거리는 **러너 × 소유 모델** 둘 다 봐야 한다 (2026-08-05 두 번 정정):
    /// ①러너: BuildDurationMult는 BuildRunner를 타는 건물 5종 전부에 걸린다 — "어느 goal에
    /// 붙었나"가 아니라 "어느 러너를 타나"가 1차 사정거리다. ②소유: 홈스테드 체제에서 모닥불·
    /// 밭·집은 **본인이 짓는 개인 소유물**이라, 건설 효율은 남에게 전이되지 않는다(사유화).
    /// 게다가 건설은 주민당 일회성이라 매일 누적되는 채집·농사·조리와 달리 며칠이면 소진된다.
    ...
    public sealed class JobSO : ScriptableObject
    {
        ...
        [Header("격퇴 계열 (M21-W8 — 헌장 §선반 자리의 입주. 부재 시나리오가 써지는 직업만)")]
        [Tooltip("전투 타격 간격 배율 (M21-W8). 1 = 현행 (무직·기존 직업 전부 불변 — 중립 불변식 " +
                 "M5-S3). 사냥꾼만 0.5 (제안치) — 무기 배율과 곱 결합 = 무장 사냥꾼 4배 (M20 배율 " +
                 "문법, 게이트 M21-T8).\n부재 시나리오: \"사냥꾼 없는 판은 격퇴가 두 배 느려 부상이 " +
                 "느는 판이다.\" 적용 지점은 FightRunner.Interval 하나 — 식사·휴식 불가침(ADR-M5-3)과 " +
                 "무관한 축이다.")]
        public float CombatDurationMult = 1f;

        [Tooltip("위협 예고 연장 (게임일, M21-W8 — 탐험가의 정찰 재탄생). 생존 보유자가 1명이라도 " +
                 "있으면 예고가 이 값만큼 이르다. ⚠️ 인원 비례 아님 — 최댓값 하나만 쓴다 (1명이면 " +
                 "족하다. 비례면 정찰꾼 몰빵 메타가 된다).\n부재 시나리오: \"정찰 없는 판은 적습을 " +
                 "하루 늦게 아는 판이다.\"")]
        public float WarnBonusDays;
