...
namespace AIVillage.M0
{
    ...
    public sealed class PersonalitySO : ScriptableObject
    {
        ...
        [Header("택지 취향 (M11-F → M11-J — 내 집이 설 자리. 기본 0 = 중립·최근접)")]
        [Tooltip("마을 앵커(기지)로부터의 **선호 거리**(체비쇼프 타일, HomePicker). 0 = 중립(최근접). " +
                 "온순 1(이웃 곁)·방랑벽 9(외딴집)처럼 성격마다 다른 동심원에 집을 짓는다. 대역(±1) " +
                 "안에서 주민 신원 기반으로 흩어져 앉는다(결정적, 랜덤 아님). 직업 값과 합산.")]
