...
namespace AIVillage.M0
{
    ...
    public sealed class ThreatSO : ScriptableObject
    {
        ...
        [Tooltip("이 종족을 **들판에 놓는가**. 🔴 '이 개체가 상주인가'가 **아니다** — 같은 종족이 " +
                 "들판에도 살고 웨이브로도 온다(고블린이 그렇다). 개체의 상주 여부는 태어날 때 " +
                 "정해지고(`ThreatAgent.IsResident`), 쳐들어온 개체는 그대로 퇴장한다.\n" +
                 "true면 **판 시작에 서식지에 태어나고 그 개체들은 퇴장하지 않는다** — 웨이브가 아니다.\n" +
        ...
        [Tooltip("**10,000타일(=100×100)당 무리 수** (상주형 전용). 0이면 상주 안 함.\n" +
                 "🔑 절대 마릿수가 아니라 **밀도**인 이유 (2026-08-11 사용자 판단): 맵은 앞으로 " +
                 "100×100보다 훨씬 넓어진다. 절대 수로 박으면 면적이 9배가 되어도 상주는 그대로라 " +
                 "**넓어진 맵이 더 안전해지는 역설**이 생긴다. 밀도면 맵을 넓혀도 코드 0줄이다.\n" +
                 "⚠️ 맵이 커지면 개체 수가 함께 늘어 `Docs/ADR_경로탐색_확장경계.md` **트리거 B**" +
                 "(맵 대형화 + 동시 AI 수십)에 닿는다 — 그때 첫 일은 쿼리 예산 스케줄러다.")]
        [Min(0f)] public float ResidentBandsPer10kTiles;

        [Tooltip("무리 하나의 마릿수 (최소~최대). 🔑 난이도의 단위는 마리가 아니라 **무리**다 — " +
                 "한 마리는 지나가다 밟히고, 3~5마리는 '돌아가야 하나'를 만든다 (사용자 판단).")]
        [Min(1)] public int ResidentBandMin = 3;
        [Min(1)] public int ResidentBandMax = 5;

        [Tooltip("무리가 흩어지는 반경 (타일). 앵커 한 칸에서 이 반경 안에 선다. " +
                 "0이면 한 칸에 겹쳐 서므로 1 이상.")]
        [Min(1)] public int ResidentBandRadius = 3;
