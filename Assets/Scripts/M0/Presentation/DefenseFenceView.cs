...
namespace AIVillage.M0
{
    ...
    public sealed class DefenseFenceView
    {
        ...
        /// 5=┌ 6=┬ 7=┐ / 8=세로하단끝 9=├ 10=┼ 11=┤ / 12=단독기둥 13=└ 14=┴ 15=┘.
        ///
        /// ⚠️ 인덱스 **뜻**은 여기 고정이고 어떤 그림을 끼울지는 에셋(TileSprites)의 몫이다.
        /// 2026-08-09 팰리세이드 교체가 그 자리를 썼다: War Camp 팩 Palisade.png는 원래
        /// **두 칸 두께 담장** 시트(가로벽 = 가시 행 + 통나무 행)라 1칸 오토타일과 문법이 다르다.
        /// 가시 얼굴 한 칸(Palisade_34)만 뽑아 가로·코너·T에 전부 끼우고, 세로 계열(0·4·8·9·11)만
        /// 통나무 기둥(Palisade_24), 단독(12)은 둥근 말뚝(Palisade_19)으로 채웠다 —
        /// 조각 종류는 16→3으로 줄지만 파생 PNG도 팩 재슬라이스도 0이다(유료 팩 픽셀을
        /// 공개 리포에 들이지 않는다). 원 시트의 코너 조각은 두 칸 두께 전제라 줄이 어긋난다.</summary>
