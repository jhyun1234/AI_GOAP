    ...
    public sealed class PlayerInputController : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            if (Input.GetKey(KeyCode.LeftControl) || Input.GetKey(KeyCode.RightControl))
            {
                ...
                // 맵 전체 공개 토글 (2026-08-12 · 사용자 요청 — 지형·바이옴을 눈으로 검사하려면
                // 주민이 밝히기를 기다릴 수 없다). 🔑 **표시 전용**이다: FoW 배열도 노드의
                // `IsDiscovered`도 안 건드리므로 끄면 정확히 원래대로 돌아오고, 그 사이 주민의
                // 판단(어디를 탐험할지·무엇을 캘지)은 하나도 바뀌지 않는다 — 관측하려던 것을
                // 관측 장치가 망가뜨리지 않게 하는 자리다.
                if (Input.GetKeyDown(KeyCode.F7) && FowManager.Instance != null)
                {
                    bool on = !FowManager.Instance.RevealAll;
                    FowManager.Instance.SetRevealAll(on);
                    dbgHud?.Notify(on ? "디버그 — 맵 전체 공개 (표시 전용 · Ctrl+F7로 복귀)"
                                      : "디버그 — 전장의 안개 복귀 (주민이 밝힌 만큼)");
                }
