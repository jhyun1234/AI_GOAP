...
3. **슬롯은 append + 예산 인지** — SlotId 예산 **72칸** (M30 증설 2026-08-12 · 51 사용 ·
   증설분 구획 장부 = `Docs/M30_슬롯예산_실행명세서.md` §2 — 광물 4·공방 6 예약). 새 슬롯은
   구획 청구 + 승인 먼저.
...
| 건물 추가 | BuildingSO + BuildActionSO 에셋. 전용 슬롯 필요 시 SlotId.cs 1파일 — 건물 1종 = 슬롯 1개(단일형 BuiltFlag 또는 수량형 Count), **슬롯 예산 72칸** (M30 §2 구획 장부 참조) |
...
- **설명 의무**: 커밋 보고 시 "게임에서 어떻게 보이는지"를 비개발자용 한 문단으로.
  🔴 **그 문단의 첫 줄은 커밋 본문 `화면:` 줄로 남긴다** (아래 "자동 커밋 + 개발 일지").
...
**커밋 본문 서사 규약 (2026-08-08)**: 이 커밋이 예상을 뒤집었다면 본문을 **믿었던 것 →
실제 관측** 순서로 쓴다. 본문은 훅을 타고 세션 로그의 "왜" 칸이 되고, 다시 블로그의
반전 재료가 된다 — 뒤집힘이 없는 커밋에 반전 서사를 억지로 붙이지는 않는다.

**🔴 커밋 본문 마지막 줄 = `화면:` (2026-08-13 신설)**: 이 커밋으로 **화면에서 사람이
무엇을 하게 됐는지**를 한 줄로 적는다. **주어는 사람, 서술어는 동사, 시제는 현재형** —
「주민이 ~한다 · ~하고 있다 · 목수가 ~을 짓는다」. 수는 괄호로 뒤에 붙인다.

- ✅ `화면: 주민이 죽으면 그 사람이 쓰던 도구가 비석 옆에 선다 (일곱 자리)`
- ❌ `화면: DeathEvent 에 이름 필드를 추가` — 코드 서술. 그건 요약 줄의 몫이다
- ❌ `화면: 사망 기록이 개선됨` — 상태 서술. 사람도 동사도 없다
- 화면이 안 바뀌는 커밋(도구·문서·테스트)은 **`화면: 없음`이 정답**이다. 지어내지 않는다.

**전과 후를 한 줄에 같이 쓴다** (2026-08-14 추가): 3D 는 인과를 **앞뒤 두 몸짓의 순서**로만
말하므로(`Docs/3D_대본_문법.md` §1), 결과만 적으면 샷이 한 장밖에 안 나온다.
「예전엔 ~했는데, 이제 ~한다」로 쓴다 — 예: `화면: 예전엔 주민이 절벽 앞에서 돌아섰는데,
이제 그 위로 걸어 올라간다`. 처음 생긴 것이라 전이 없으면 후만 쓴다.

**3D 어휘 밖이면 `화면: 없음`이다** (2026-08-14 추가): 그 줄이 `Docs/3D_대본_문법.md` §3 의
**동작 22종 · 소품 목록** 안에서 말해지지 않으면 — 주어가 사람이 아니거나(지형·타일·색),
없는 것을 부르거나(글자·표·그래프) — 화면이 바뀌었어도 **`화면: 없음`이 정답**이다.
억지로 쓰면 영상 트랙이 못 찍는 줄을 받는다. 이 줄이 곧 영상 태그(`vid/*`)의 판정 기준이다.

**왜 본문에 박나**: 본문은 훅을 타고 세션 로그 "왜" 칸에 그대로 실리고, 거기서 블로그 →
쇼츠·롱폼 대본 → 3D 샷으로 내려간다. 3D 는 **사람이 하는 일**로만 말할 수 있어서, 이
사슬 맨 위에서 동사가 안 나오면 맨 아래에서 화면이 빈다. 종전 「설명 의무」(아래)는
말로만 하고 사라졌다 — **그 문단의 첫 줄을 커밋에 남기는 것**이 이 규약이다.
...
| **뒤집힌 것 (믿었던 것 → 실제)** · **육성 인용** | **사람/Claude** — 재미 포집 칸 (2026-08-08 신설). 파이프라인은 재미를 지어낼 수 없어 여기 없는 반전·육성은 글에 못 실린다. **없으면 "없음"이 정상 답** — 지어내기 금지 |
