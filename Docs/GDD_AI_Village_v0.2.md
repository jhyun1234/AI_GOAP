...
loyaltyLevel     : float   // 0~100 (충성도 — 명령 수행 의지)
hasTool          : bool
hasWeapon        : bool
hasPrimitiveWeapon: bool   // v0.2 추가
hasFood          : bool
atBase           : bool
nearResource     : bool
nearEnemy        : bool
...
// 마을 전역 상태 (Global)
foodStock        : float   // 가용량 = stock - reserved
woodStock        : float
stoneStock       : float
ironStock        : float
copperStock      : float
silverStock      : float
// 자원 예약 (Resource Reservation System — v0.2 추가)
foodReserved     : float
woodReserved     : float
stoneReserved    : float
ironReserved     : float
...
| P4 | GatherResources | anyAvailableStock < 30 (식량 포함) |
| P5 | Explore | allAvailableStocks >= 50 |

> **v0.2 변경:** P0 세 Goal의 서브 우선순위 확정 (SurviveInjury > SurviveHunger > SurviveFatigue). foodStock이 anyAvailableStock 조건에 명시적 포함.
...
> **v0.2 변경:** RestOnGround Cost 3→**12**, 효과 -40→**-20**. Sleep(Cost 10)이 GOAP 플래너에 의해 자연스럽게 선호되도록 조정. 도저히 수면 불가 시에만 마지막 수단으로 사용.
...
> **v0.2 변경:** Town Hall 비용 Wood 50→**35**, Iron 10→**6** (초반 병목 완화).
...
> **v0.2 추가:** CraftPrimitiveWeapon — Forge 없이 즉시 제작 가능한 원시 무기. 소형 동물(위협 1등급)에만 유효. Day 1 GOAP Dead-end(DefendVillage Goal 달성 불가) 해소.

### 3.4 GOAP 플래닝 예시

**시나리오:** Day 1, 주민 'Arin' hungerLevel=78 → Goal: SurviveHunger

```
경로 A (Cost 2):  EatStoredFood — hasFood=true, atBase=true ✅ → 즉시 실행
경로 B (Cost 8):  HarvestWildBerries → (이동 Cost+3) = 8
경로 C (Cost 18): HarvestWildBerries → ReturnToBase → CookMeal

최적: 경로 A 선택. foodStock=0이면 경로 B로 자동 Re-plan.
```

**시나리오:** Day 1, 늑대 출현 → Goal: DefendVillage (v0.1에서 Dead-end 발생)

```
v0.1: AttackEnemy Precondition: hasWeapon=true ❌ → Forge 없음 → Dead-end
v0.2: CraftPrimitiveWeapon (wood>=3, stone>=2) → hasPrimitiveWeapon=true
      → AttackEnemyWeak → enemyDefeated=true ✅
      체인: CraftPrimitiveWeapon → AttackEnemyWeak, 총 Cost 16
      (체력 < 40이면 FleeFromEnemy 선택, Cost 3)
```
...
| 충성도(Loyalty) | 0~100 | 구간별 효과 (아래 참조) | 명령 수행 의지 |
...
### 4.3 충성도(Loyalty) 시스템 — v0.2 Hybrid C+D 방식

#### 레이어 1: GOAP Cost Modifier (방식 C)
ExecutePlayerOrder Action의 실효 비용을 충성도에 따라 조정한다. GOAP 플래너가 자연스럽게 명령 수행 여부를 결정.

| 충성도 구간 | Cost 배율 | 동작 |
|---|---|---|
| 70~100 | ×0.7 | 명령 수행 적극적, 모든 Cost -10% 보너스 |
| 50~69 | ×1.0 | 기본값 |
| 30~49 | ×2.5 | 거부 빈도 증가, 모든 Cost +10% |
| 0~29 | ×6.0 | 사실상 명령 무시, 마을 이탈 위험 |

#### 레이어 2: Need-Conflict Score (방식 D)
명령이 현재 필요와 얼마나 충돌하는지 점수화하여 명시적 거부 결정.

```
ConflictScore = Σ (Need_urgency[i] × Order_impact[i])

예시: hunger=75인 주민에게 "벌목 명령"
  hunger_urgency    = 75 / 100 = 0.75
  chop_hunger_cost  = +3 (벌목 시 허기 추가 증가)
  ConflictScore     = 0.75 × 3.0 = 2.25

실효 거부 임계값 = 2.5 × (loyaltyLevel / 50)
  loyalty=100 → 임계값 5.0 (관대)
  loyalty= 50 → 임계값 2.5 (기본)
  loyalty=  0 → 임계값 0.0 (거의 항상 거부)

ConflictScore < 임계값 → 명령 수행
ConflictScore ≥ 임계값 → 명시적 거부 + UI 메시지 표시
```
...
| loyalty < 30 (자율 의지) | "왜 제가 그걸 해야 하죠?" | P4~P5 자율 수행 |
...
| 자원 | 획득 방법 | 주 용도 | 희소성 |
|---|---|---|---|
| 나무(Wood) | ChopWood | 건물 기본재, 연료 | 낮음 |
| 돌(Stone) | MineStone | 건물 강화재 | 낮음 |
| 철광석(Iron) | MineIron | 도구·무기·고급 건물 | 보통 |
| 구리(Copper) | MineCopperOre (Forge 필요) | 고급 건물 | 높음 |
| 은(Silver) | MineOre (위험 지역) | 최상위 건물 | 매우 높음 |
| 식량(Food) | Harvest/Hunt/Cook | 주민 생존 | 소모성 |

### 5.2 건물 건설 비용표
| 건물 | Wood | Stone | Iron | Copper | Silver | 기능 |
|---|---|---|---|---|---|---|
| House | 20 | 10 | 0 | 0 | 0 | 수면 효율 +60%, Loyalty +5/일 |
| Storehouse | 15 | 5 | 0 | 0 | 0 | 자원 용량 기본 100 → **200** |
| Campfire | 5 | 0 | 0 | 0 | 0 | 요리 가능, 난방 |
| **Town Hall** | **35** | 30 | **6** | 0 | 0 | 팩션 기능 해금, 주민 모집 |
| Forge | 20 | 20 | 15 | 0 | 0 | 무기·도구 제작 — **마일스톤 이벤트 발동** |
| Watchtower | 10 | 30 | 5 | 0 | 0 | 적 탐지 범위 ×2 |
| Communication Tower | 30 | 20 | 10 | 20 | 0 | 외교·교역 해금 |
| Alchemist Lab | 15 | 10 | 5 | 5 | 0 | 의약품·식량 보존 기술 제작 |
| Silver Citadel | 60 | 50 | 30 | 20 | 15 | **최종 승리 건물** |

> **v0.2 변경:** Town Hall Wood 50→35, Iron 10→6. Storehouse 용량 수치 명시 (100 units, Storehouse 건설 시 200).

### 5.3 경제 수치

**게임 시작 스타팅 보너스 (v0.2 추가):**
```
초기 지급: 식량 30 units (Day 1~3 버퍼)
...
**Town Hall 건설 실질 예상 시간 (v0.2 수정 기준):**
```
주민 3인, 1인 식량 전담, 2인 건설 자원 수집:

Wood 35 ÷ (8 × 2인) = 약 2.2일
Stone 30 ÷ (5 × 2인) = 약 3.0일  ← 병목
Iron 6  ÷ (3 × 2인) = 약 1.0일

병렬 수집 시 (Wood+Stone 동시): 약 3일 달성 가능
→ v0.1의 현실적 최악치 15일에서 6~7일로 개선
```

### 5.4 겨울 위기 대응 시스템 (v0.2 추가)

```
겨울 경보 임계값:
  foodStock < (주민 수 × 3 × 30일) → "겨울 대비 경보" UI 표시
  예: 주민 5인 → 경보 발동 기준: foodStock < 450

Alchemist Lab 제작: 식량 보존 기술
  효과: 겨울 식량 소비 -20%
  비용: 구리 3 + 식량 5 (원재료)
  목적: 겨울 4일 고갈 → 5일 이상으로 완화

계절별 긴급 자동 배치:
  가을 시작 시 GOAP GatherResources Goal에 "식량 가중치 × 3" 자동 적용
  → 주민들이 자율로 식량 비축 행동 우선
```
...
```
침략 결정 조건:
  (copperStock < 10 OR silverStock < 5)   ← v0.2: 은 조건 추가
  AND nearPlayerTerritory == true
  AND playerStrength < factionStrength × 0.8  ← v0.2: 전력 비교 추가

→ RaidDecision = true → ExpandTerritory Goal 활성화
→ 전력 열세 시: AllianceProposal 또는 교역 제안 우선
```

### 6.3 유닛 상한선 및 LOD AI (v0.2 신규)
...
LOD AI 규칙:
  ...
  전환: Full GOAP → 간소화 FSM (비용 ~0.01ms/유닛)
  복귀: 전투 참여 시 또는 30타일 이내 진입 시 Full GOAP 재활성화

예상 성능:
  실질 Full GOAP 동시 유닛: 약 60~70명
  프레임당 GOAP 처리 비용: ~1.8ms (Job System + Burst 적용)
  60fps 목표 달성 가능 (전체 AI 예산 < 4ms)
```
...
> 구리·은은 의도적으로 무주지/적 영토에 집중. 침략 동기 부여.
...
| 봄 | +20% | 없음 |
| 여름 | +30% | 화재 위험 ↑ |
| 가을 | +10% | 수확 이벤트 (+50 식량), **식량 가중치 ×3 자동 적용** |
| 겨울 | -50% | 연료 소비 ×2, 피로 증가 +30%, **겨울 경보 시스템 작동** |

---

## 9. 마일스톤 이벤트 시스템 (v0.2 신규)
...
- Q04: 주민 사망 시 인벤토리 드롭 여부
- Q05: 주민 신규 모집 비용 및 초기 속성
- Q06: 적 팩션 AI 초기 자원 상태 및 기지 위치
- Q07: Fog of War 하에서 GOAP 미탐험 자원 처리 방식
- Q12: 자원 노드 재생 여부 (나무 재생? 광물 영구 고갈?)

---

*작성일: 2026-06-25 | 버전: v0.2*
*다음 버전(v0.3) 목표: Q04~Q07 확정 반영, 주민 간 상호작용 설계, 튜토리얼 온보딩 설계*
