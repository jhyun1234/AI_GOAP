...
**마지막 사전점검**: 2026-07-16 (자동 스케줄, 13:03 발행 1시간 전) — 게이트 재계산
(A=45커밋≥10, B=대형7개&45≥5 전부 통과) → 오늘 발행 예상. **순서 역전 위험 발견 및 교정**:
`blog-planner.md`의 소재 스코핑이 "마지막 *발행일*"(2026-07-15 14:23, M0 회고) 기준이라
그보다 이전 시각(2026-07-14~07-15 00:10)에 완결된 M2/M3를 건너뛰고 바로 M4/M5로 갈
위험 확인 → `state/blog_next_material_priority.md`를 M2+M3 소재로 **ACTIVE 지정**
(지난 회차가 "다음다음 회차 후보"로 관찰만 해둔 항목을 이번에 활성화). #4(M0 회고) 글
사실관계 스팟 체크(24,800→5,977줄, -76%, 59개 파일 삭제) 로컬 아카이브 대조 완료 — 오류
없음. M4(성격 재편입)+M5(직업 스펙, dd27f30) 신규 완결 구간 발견, 표에 추가(교정 불필요 —
M2/M3 다음 관찰 대상). 파이프라인 경보: state push 403 수정(19c9728)의 검증 대상 run이
바로 오늘 13:03 — 사전점검 시점엔 아직 결과 없음, 사후 확인 필요.
...
| 4 | (차기 예약) | **M0 재설계 회고** (24,800→5,977줄) | 8ee845b~b175ddf +04f0975 | 3번의 앞 이야기 (회고 특집으로 복구) | PRIORITY ACTIVE — 오늘(2026-07-15) 13:03 게이트 통과 예상 (기준 A/B/D 전부 충족) |
| — | 미발행 구간 | 옵시디언 도입·블로그 게이트 전환 등 운영 개선 | f71c0a8, acd0e13 등 | 소재 가치 낮음 — 스킵 허용 | 소비됨 |
| 5 | (차기 예약) | **M2 생산체인(농사·요리) + M3 집과 기틀** — 밭·부엌·앵커 승격 체인, 집/통행차단/작업클레임 | 651ea47~153f180 | 4번(M0) 다음 소재 — 두 밀스톤 다 spec+feat+test 완결, 이야깃거리 충분 | PRIORITY ACTIVE — 2026-07-16 사전점검 지정 (순서 역전 방지, `blog_next_material_priority.md` 참조) |
| — | 미발행 신규 구간 (2026-07-16 사전점검 발견) | **M4 주민의 성격(아키타입 재편입) + M5 직업과 일과(스펙만)** — 성격 계수·혼잣말·아키타입, 성격x직업 조합 명세 | b982090~862de3b (M4), dd27f30 (M5 spec) | 5번(M2+M3) 다음 소재 — M4는 완료, M5는 스펙만 확정(구현 전) | 대기 (다음다음 회차 후보로 관찰만, 아직 ACTIVE 지정 안 함) |
