...
2. **blog-writer** — 기획팀 브리프로 초안 작성.
   - 🔴 **작성팀 호출 직후 오케스트레이터가 직접 분량을 계측한다** (2026-07-28 추가):
     ```bash
     wc -m < tools/blog-automation/.staging/02_draft.md
     ```
     **4,000~15,000 밴드를 벗어나면 검수팀을 부르지 말고 작성팀에 바로 돌려보낸다** —
     숫자를 알려주고 축약/보강만 요청한다. 이건 반려가 아니므로 **카운터를 올리지 않는다.**
     작성팀은 도구가 `Read`/`Write`뿐이라 `wc -m`을 실행할 수 없어 자기 분량을 모른다
     (07-28 실측: 작성팀 추정 12,000자 → 실제 23,351자). 객관적으로 측정 가능한 미달을
     검수 반려로 소비하면 카운터 3칸이 금방 사라진다.
