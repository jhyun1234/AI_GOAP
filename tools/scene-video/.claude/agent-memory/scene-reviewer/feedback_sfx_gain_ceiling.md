...
`tick(0.052s/1180Hz)` **0.05979** · `tick(0.05s/1240Hz)` **0.056~0.057 사이**(lf02 S3) ·
`sweep(0.58s/seed 43)` 0.18435.

🟡 **걸렸다고 무조건 재렌더를 요구하기 전에 「고치면 얼마나 달라지는가」를 재라**(lf02 에서 확립).
lf02 S3 은 `tick(0.05/1240)` 이 gain 0.060 에서 걸려 실효 **0.0578(−0.3dB)** 이 됐는데,
규칙대로 `0.056` 으로 내리면 실효가 **0.0560** 이라 **지금보다 0.27dB 더 조용해진다.**
「그 소리만 조용해진다」는 이 규칙의 취지가 겨냥하는 불균형이 0.3dB 이면 고친 결과와
구분되지 않는다 → **값은 고쳐 두되 그것만으로 22분 재렌더를 시키지는 않는다.**
(반대로 −1.1~1.8dB 급이면 그건 진짜 불균형이다 — `sfx.mjs` 주석의 옛 실측이 그 규모다.)
