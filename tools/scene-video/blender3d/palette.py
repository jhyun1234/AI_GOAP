...
# 인물 몸의 알베도 — **세계층 무채색**이다(채도 0 이라 뜻 예산에 안 들어간다).
# `mixamo.build` 가 Y Bot 을 이 값으로 다시 칠한다 — Y Bot 은 **청록색**으로 오는데
# 그 색이 화면에 들어오면 뜻층 다섯과 경쟁한다.
# 🔴 `stage.INK_LIN`(흰 도형 0.62)과 **다른 값이다.** 같은 값을 주면 인물이 순백으로
#    클리핑한다 — 실측 37.9%. 도형은 평평해서 정면광을 한 면으로만 받고, 사람은 굽은
#    면이라 태양(5.0×1.85)을 정면으로 받는 부분이 그대로 1.0 을 넘긴다.
FIGURE_LIN = (0.30, 0.30, 0.305)
FIGURE_ROUGHNESS = 0.85        # 반들거리면 플라스틱이 된다. 하이라이트가 클리핑의 절반이었다
