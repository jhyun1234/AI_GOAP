...
# 🔑 앞 판은 넷을 나눠 가졌다(farm 둘 · stop 하나). 어휘가 없어서 겹친 것이지 그게 뜻은
#    아니었다 — **갈라짐이 사건인데 둘이 같은 일을 하면 그만큼 덜 갈라진다.** 어휘가 생겼으니
#    여섯이 여섯을 갖는다. 🔴 이건 「어휘를 늘리자」가 아니다. 늘어난 어휘를 **여기서 쓰는 것**이다.
# 🔴 자리가 없어도 읽히는 것만 고른다. 갈라진 여섯은 마을 아무 데나 흩어지므로
#    덤불·우물 같은 **대상이 필요한 동작은 여기 못 온다**(`gather` 를 넣었다가 허공을 젓는다).
#    `chop` 만 예외인데, 도끼를 **손에 쥐여 주기 때문**이다(render_body3 이 붙인다).
BREAK = ['farm', 'chop', 'draw', 'walk', 'look_around', 'run']
...
def march(t, beat):
    ...
    for i, kind in enumerate(HOOK_BEATS):
        ...
        if kind == 'walk':
            ...
            # 🔑 `_snappy` 라 **반대로 살짝 틀었다가**(예비 동작) 빠르게 돌고 오버슛했다 앉는다.
            #    `_ease` 판은 여섯이 동시에 매끈하게 도는데, 그 매끈함이 곧 「기계적」이었다.
            # 🔴 u ≥ 1 에서 정확히 1 이라 끝 각은 여전히 HOOK_TURN 이다(검사가 1e-9 로 본다).
            heading += HOOK_TURN * motions._snappy(u / (beat * TURN_SPAN))
...
# 「군무가 깨진다」에서 여섯이 **걸어 나가는 쪽**(본문3 ⑦ · 아웃트로가 이어받는다).
# 🔴 **집 쪽(+Y)으로 보내지 마라.** 마을 북쪽은 집 셋이 벽처럼 서 있다(y 2.2~4.0). 무리
#    중심에서 방사로 보냈더니 주민 0 이 집 둘 사이에 끼어서, 걸어 나가는 것도 아웃트로에서
#    돌아다니는 것도 자리가 안 나왔다(궤적 검사가 -0.69 m 로 잡았다).
# 🔑 열린 곳은 카메라 쪽(-Y)과 좌우다. -40°~-140° 부채꼴로 보내면 여섯의 간격이
#    0.75~1.23 로 남는다(원래 0.95) — 「갈라졌다」가 「뭉쳤다」로 안 뭉개지는 하한이 0.6 이다.
# 🔴 각을 손으로 흔들지 마라. 흔들면 `render_outro.ring_for` 가 자리를 못 찾고 렌더가 죽는다.
SPLIT_FAN = [math.radians(-40 - 20 * i) for i in range(N)]
