...
def sword(s=1.0):
    """검 — 전쟁 축(M32) · `AnimKind.Slash`·`Thrust`. 자루 + **날밑** + 길고 납작한 날.

    🔑 이 물건을 검으로 만드는 것은 **날밑(crossguard)** 이다. 자루와 날만 있으면 그건
       칼이 아니라 **말뚝**이다 — 도구 넷이 「면·뿔·짧은 자루·주둥이」로 갈리는 그 자리다.
    🔴 날을 원기둥으로 두지 마라. **납작해야** 벤다는 뜻이 선다(도끼 날과 같은 규칙) —
       X 로 눌러 넓은 면이 휘두르는 평면을 향하게 한다.
    🔴 자루는 **+Y**다(도구 넷과 같은 규약). 활만 축이 다르다(아래 `bow` 참조).
    🔴 **날을 원뿔로 두지 마라**(첫 판이 그랬다). 자루부터 뾰족해지면 그건 검이 아니라
       **단검**이다 — 검은 날이 거의 **나란하다가 끝에서만** 좁아진다. 판 + 끝 쐐기다.
    """
    m = mats()
    guard = _cube((0, 0.105 * s, 0), (0.090 * s, 0.020 * s, 0.024 * s), m['stone'])
    blade = _cube((0, 0.270 * s, 0), (0.056 * s, 0.310 * s, 0.017 * s), m['stone'])
    tip = _cone((0, 0.455 * s, 0), 0.028 * s, 0.070 * s, m['stone'], verts=4)
    tip.rotation_euler = (math.radians(-90), 0, 0)      # +Z 로 선 원뿔을 +Y 로 눕힌다
    tip.scale = (1.0, 1.0, 0.60)                        # 날 두께에 맞춰 납작하게
    return _tool('sword', [_haft(0.115 * s, m['earth'], -0.020 * s), guard, blade, tip])


def bow(s=1.0):
    """활 — 전쟁 축(M32) · `AnimKind.Shoot`. **굽은 활채 + 곧은 시위.**

    🔑 이 물건을 활로 만드는 것은 **활채가 아니라 시위**다. 굽은 막대만 그리면 그건
       지팡이다 — 활은 **활 모양(호)과 그 호를 가로지르는 직선**이 같이 있을 때만 읽힌다.
       도끼가 「면」, 곡괭이가 「뿔」, 망치가 「짧은 자루」로 갈리는 것과 같은 자리다.
    🔴 활채는 **쏘는 사람 쪽으로** 휜다(시위를 건 활의 모양). 반대로 휘면 안 걸린 활이라
       시위가 활채 뒤로 떨어져 나가고, 그러면 실루엣이 「막대 옆의 줄」이 된다.
    🔴 잡는 자리가 **가운데**다. 도구 넷은 자루 끝을 쥐지만 활은 허리를 쥔다 — `_haft` 를
       쓰지 않는 유일한 소품인 이유다.
    🔑 시위는 얇지만 **세계층 밝은 재질**(`stone`)을 쓴다. 활채와 같은 색이면 호 안에서
       사라진다 — 이 물건에서 사라지면 안 되는 것이 바로 그 직선이다.
    🔴 유채색 금지 — 도구와 같은 세계층이다.
    🔴 **곧은 뿔 하나로 활채를 만들지 마라** (첫 판이 그랬다). 곧은 것 둘에 직선 하나를
       걸면 활이 아니라 **단검**으로 읽힌다. 마디를 각도를 키우며 이어 **호**를 만든다 —
       도끼 날을 상자로 두면 주걱이 되는 것과 같은 자리다.
    """
    m = mats()
    # 마디 (길이, +Z 로부터의 각도°, 반지름) — 각이 커질수록 끝이 쏘는 사람 쪽으로 감긴다.
    SEGS = ((0.060, 10, 0.013), (0.058, 26, 0.011), (0.052, 44, 0.009))
    parts = [_cube((0, 0, 0), (0.026 * s, 0.028 * s, 0.075 * s), m['earth'])]   # 허리(쥐는 곳)
    tip_y = tip_z = 0.0
    for sign in (1, -1):                                   # 위 짝 · 아래 짝 (z 대칭)
        y, z = -0.006 * s, 0.037 * s * sign
        for length, deg, r in SEGS:
            a = math.radians(deg)
            dy, dz = -math.sin(a) * length * s, math.cos(a) * length * s * sign
            seg = _cyl((0, y + dy / 2, z + dz / 2), r * s, length * s, m['earth'], verts=4)
            # 실린더는 +Z 로 선다. R_x(a) 가 (0,0,1) → (0,−sin a, cos a) 라 그 각이 곧 방향이다.
            seg.rotation_euler = (a if sign > 0 else math.pi - a, 0, 0)
            parts.append(seg)
            y, z = y + dy, z + dz
        tip_y, tip_z = y, abs(z)
    # 시위 — 활채 **끝에서 역산한다.** 눈대중으로 적으면 활채에서 뜨거나 파묻힌다.
    parts.append(_cyl((0, tip_y, 0), 0.004 * s, tip_z * 2, m['stone'], verts=4))
    # 🔴 **활채를 로컬 X 로 눕혀서 넘긴다** (2026-08-16 실측 — 손에 붙여 보고 알았다).
    #    `stage.hold` 은 `rot_x` 하나만 받는데, 활채를 Z 로 두면 그 회전이 활채를 **팔 방향
    #    평면 안에서만** 돌린다 — 0°·180° 는 카메라를 향해 눕고 90°·270° 는 팔과 나란해져서,
    #    **어느 각에서도 활이 안 선다.** 뼈 로컬에서 세로인 축은 X 라 거기에 실어야 한다.
    #    (도구 넷은 자루가 +Y 라 이 문제가 없었다 — 활만 축이 다른 유일한 소품이다.)
    inner = _tool('bow_limbs', parts)
    inner.rotation_euler = (0, math.pi / 2, 0)     # Z → X
    return _tool('bow', [inner])
