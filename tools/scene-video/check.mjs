...
/* ── 제목 후킹 — 제목에 아라비아 숫자가 있는가 (2026-08-06 신설) ────────
   위 게이트는 **약속을 지키는지**만 본다. 약속이 매력적인지는 여태 아무도 안 봤다.
   그래서 이행은 통과하는데 아무도 안 누르는 제목이 계속 나갔다.

   관측: 채널 제목 14편을 전수 분류하니 아라비아 숫자가 든 제목은 **3편뿐**이다
   (ep01s 4,096 · ep03s 24,800→5,977 · ep05s-3 0줄). 외부 채널 분석이 클릭을 끈
   제목으로 지목한 둘이 그 셋 안에 있고, 나머지 하나(ep01s)는 2026-08-03 리포에서
   지속률 2위다. 나머지 11편은 숫자가 없고 최근 4편은 **자막 본문에도 숫자가 0개**였다.

   🔴 재료가 없어서가 아니다. 발행 글 10편을 세어 보니 수치 문장이 글당 2~37개 있고
   그 대부분이 표 밖 산문이다(기획팀의 표 삭제와 무관하다). ep06s 는 원문에 있던
   「5번째 아키타입은 코드 0줄로 추가됐습니다」를 손에 쥐고도 "직전 편과 같은 훅"이라며
   버렸다(episodes/ep06s/notes/writer.md:32). 먹히는 훅을 파이프라인이 스스로 봉인했다.

   ⚠️ 이 검사는 숫자의 **존재**만 본다. 숫자가 좋은지는 못 본다 — 그건 작성팀 규칙
   (scene-writer.md 「제목 후킹」)이 맡는다. 여기는 바닥이지 천장이 아니다.
   ⚠️ 편당 CTR 실측은 아직 없다(채널 집계 3.96% 한 개뿐). 근거는 위 텍스트 전수와
   외부 분석이다. 뒤집히면 게이트를 무르게 하지 말고 근거째로 다시 써라(ADR-V-7).
