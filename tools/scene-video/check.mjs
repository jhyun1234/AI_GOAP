...
/* ── 제목 후킹 — 제목에 아라비아 숫자가 있는가 (2026-08-06 신설 · 같은 날 warn 강등)
   ...
   🔴 무르게 한 것이 아니라 **겨냥하는 지표를 정정한 것이다.** 검색·홈 유입이 자라서
   노출수가 유의미해지면 그때 fail 로 되돌릴 근거가 생긴다(ADR-V-7).
   🔴 과거 14편에 돌리면 11편이 걸려야 옳다. 전부 통과하면 검사가 망가진 것이다. */
...
add(titleDigits.length > 0, '제목 후킹 (수치)',
  titleDigits.length
    ? `숫자 ${titleDigits.join(', ')}`
    : `🟡 제목에 아라비아 숫자가 없다 — 「${titleBody}」`
    + (koNums.length
      ? ` · 한글 수사 [${koNums.join(', ')}] 를 숫자로 바꿔라 (다섯 명 → 5명)`
      : ` · 원문의 수치 문장에서 하나 끌어와라 (줄 수·개수·배수·초)`),
  'warn');
...
/* 🔴 2026-08-06 개정 4.0 → 1.0 (Docs/영상_25초_전환_실행명세서.md W2·ADR-V25-2).
   ...
   **두 형태를 다 받는다.** 어느 쪽인지는 회차마다 작성팀이 고른다.
   ...
   🔴 **1.0 → 1.8 (2026-08-06 사용자 판정, ep08s-2 실측).** 1.0 은 **물리적으로 달성 불가능한
   값이었다.** TTS 실측에서 본문 줄은 179~213ms/자로 균일한데 짧은 줄에는 **고정 오버헤드가
   약 550ms** 붙는다(ep08s-2 예고 6자 = 1,657ms = **276ms/자**). 그래서 1.0초 안에 들어가는
   발화는 **최대 2.4자**이고, 「다음 편.」 세 글자도 1.1초가 나온다 —
   **뜻이 있는 음성 훅은 1초 안에 물리적으로 안 들어간다.**
   1.8 은 그 실측에서 나온 하한이다(6자 = 1.66초). 옛 4.0 의 절반 아래라 「쇼트 훅」 취지는
   그대로 살고, 경고는 **진짜 긴 예고**(3초짜리 옛 형식)만 잡는다.
   🔑 이건 게이트를 무르게 한 것이 아니라 **달성 불가능한 값을 실측으로 교정한 것**이다.
   항상 뜨는 경고는 읽히지 않으므로, 지키는 회차와 안 지키는 회차를 가르지 못하면 게이트가 아니다. */
const TEASER_MAX = 1.8;
...
if (timed) {
  ...
  /* 🔴 권장대역 하한이 20 → 22.6 (2026-08-06 리뷰①). 발명한 값이 아니라 **파생값**이다:
     본문 하한 20 + 아웃트로 2.6 = 22.6. 옛 값 20 은 아래 「본문」과 모순이었다 —
     20~22.6초 회차는 이 항목을 통과하면서 본문 항목에서 걸리는 죽은 구간이었다. */
