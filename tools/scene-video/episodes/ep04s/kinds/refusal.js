...
/* refusal — 명령이 주민 앞에서 자기 상태에 걸린다.
   ...
   지난 편의 마지막 그림이 '명령이 다가왔다가 주민 앞에서 밀려나는' 한 획이었다. 이 편은
   그 획을 받아서 **왜 밀려났는지**를 연다. 명령 칩이 오른쪽에서 주민에게 다가오고
   (orderCue), 주민이 자기 상태 둘을 꺼내 보고(statCue), 두 문턱이 숫자로 채워지고
   (thresholdCue), 조건에 걸리면 말풍선이 뜨면서 칩이 되밀린다(refuseCue).

   🔴 말풍선 문구는 원문 그대로다. 이 편에서 화면에 올릴 것이 딱 하나라면 이 두 마디다 —
   나레이션은 둘 중 하나만 읽고, 나머지 하나는 화면에서만 읽힌다. 시청자가 스스로 읽은
   문장이 남는다.

   마지막에 세 번째 갈래가 열린다(notRandomCue). 거부만 있는 판정이 아니라는 것을
   화면에서도 세 갈래로 보여야 '랜덤이 아니라 상태 판정'이 성립한다.
...
export default {
  ...
  draw(root, { spec, t, cue }) {
    ...
    const ok = ease(cue(spec.orderCue ?? 0, 0.15, 0.6));
    const sk = ease(cue(spec.statCue ?? 1, 0.15, 0.6));
    const tk = ease(cue(spec.thresholdCue ?? 2, 0.15, 0.6));
    const rk = ease(cue(spec.refuseCue ?? 3, 0.15, 0.5));
    const nk = ease(cue(spec.notRandomCue ?? 4, 0.15, 0.55));
