...
/* sprout — 밭이 화면에서 실제로 자란다.
   ...
   이 편(5-1)의 **세 번째 샷**이다. 앞 샷(worn)에서 아무도 가지 않던 빈 칸 「밭」이
   여기서 처음으로 채워진다 — 그래서 이 샷의 배경은 점선 빈 칸이 아니라 흙과 이랑이다.
   ...
   🔴 "어느 순간에 멈춰도 세 단계가 동시에 보인다"고 쓰면 거짓이다. '씨앗'(길이 0.28)과
   '자라는 중'(길이 0.32)은 간격 1/3 = 0.3333 보다 좁아서 각각 시간의 16.0% · 4.0% 동안
   0 포기다. 그래도 이 순환은 자막에 물릴 사건이 아니다 — 어떤 단계도 '처음 등장'하지
   않고 계속 순환할 뿐이라 도착 시각이라는 것이 없기 때문이다. 위 밭은 배경이고,
   세 단계에 **이름을 붙이는 것은 아래 단계 띠**다. 띠의 세 칸은 고정 위상
   u = 0.10 · 0.44 · 0.74 로 세 단계를 항상 그려 두므로 자막(stageCue)이 이름을 부르는
   순간 셋이 반드시 화면에 함께 있다.
   문턱 0.28 을 0.34 로 올려 씨앗 구간도 1/3 을 넘기는 안은 쓰지 않았다 — 그러면
   '자라는 중'이 0.26 으로 줄어 이번엔 그쪽이 더 오래 비고, 밀의 모양도 씨앗에 치우친다.

   원문 3절의 "FarmService로 농사 로직", "Kenmi 밀 스프라이트 3단계(씨앗→자라는 중→
   수확 가능)"가 근거다. 화면의 숫자는 단계 셋뿐이고 수확량·주기 같은 값은 하나도 없다 —
   원문이 그런 수를 대지 않기 때문이다.

   계속 도는 것 = 밀 셋의 성장 순환과 잎의 흔들림. cue 와 무관하게 t 로만 돈다. */
...
export default {
  ...
  draw(root, { spec, t, cue }) {
    ...
    const gk = ease(cue(spec.growCue ?? 0, 0.15, 0.60));
