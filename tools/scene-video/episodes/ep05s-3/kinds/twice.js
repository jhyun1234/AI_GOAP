...
/* twice — 떨어져 있던 자국 둘이 가운데로 모여 정확히 포개진다.
   ...
   🔴 훅(scene.hook)을 예고 자막이 아니라 **meanCue** 에 물렸다. 이유는 정적 구간이 아니라
   아웃트로 카드다 — engine.js:174 의 OUTRO_MS 2600 때문에 마지막 자막 시작 +0.7초쯤부터
   카드가 .vis 를 통째로 덮는다. 훅을 예고 자막에 걸면 시청자는 그 한 줄을 0.7초만 본다.
   대신 예고 자막에는 좁은 창(span 0.18)으로 튀어 오르는 다음 편 칩을 걸었다 — 카드가
   덮기 전에 뜨고, 예고 구간에도 cue 로 움직이는 것이 하나 남는다.
...
export default {
  ...
  draw(root, { spec, scene, t, cue }) {
    ...
    /* ── 남는 한 줄 ───────────────────────────────── */
    /* ep01s erasure 가 만들고 네 편이 이어 온 자리다. 이 편은 예고 샷 없이 twice 로 끝나므로
       같은 블록이 여기 있다. meanCue 에 물린 사유는 머리말 참조(아웃트로 카드가 덮는다). */
