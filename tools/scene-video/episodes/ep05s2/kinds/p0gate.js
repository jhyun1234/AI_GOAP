...
/* p0gate — 벽 전체를 치우지 않고, 두 목표에게만 틈을 낸다.
   ...
   🔴 직전 편이 쿨다운을 '닫힌 고리에 걸리는 걸쇠'로 그렸다. 여기서는 고리를 한 번도
   쓰지 않는다. 이 편의 쿨다운은 돌아가는 것이 아니라 **가로로 누워 길을 막는 것**이고,
   사건은 그 위를 도는 것이 아니라 **뚫고 내려가는 것**이다.
...
export default {
  ...
  draw(root, { spec, t, cue }) {
    ...
    const bk = ease(cue(spec.blockCue ?? 0, 0.15, 0.6));
    const pk = ease(cue(spec.skipCue ?? 1, 0.15, 0.6));
    ...
    /* ── 쿨다운 띠 — 틈을 뺀 나머지 구간만 그린다 ── */
    {
      ...
      ctx.fillText(spec.barrier || '실패 쿨다운', 22, BAND_Y - 9);
      ctx.globalAlpha = 1;
    }
    ...
    if (pk > 0.55 && spec.fireLabel) {
      ...
      ctx.fillText(spec.fireLabel, w / 2, 288);
      ctx.globalAlpha = 1;
    }
