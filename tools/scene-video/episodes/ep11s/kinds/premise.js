...
/* premise — 위층을 아무리 다시 조준해도, 금이 가는 건 아래층이다.
   ...
   ① 앞쪽 — 목수에서 조준선이 네 번 뻗는데 **매번 같은 자리에서 멈춘다.** 멈춘 자리에
      짧은 눈금이 하나씩 쌓여 넉 줄이 된다 — 다시 조준한 횟수는 눈에 보이되 숫자로 쓰지 않는다
      (원문에 시도 횟수가 없다). 조준선 끝과 의뢰인 사이는 끝까지 비어 있다.
   ② 뒤쪽 — 위층이 흐려지고, 아래 판이 좌우로 벌어진다. 벌어진 **검은 틈 안에서만**
      강조색 균열이 위에서 아래로 번진다. 판 위의 글자도 함께 찢어진다.

   🔴 **2026-08-08 2차 개정 — 자막 두 줄이 한 줄로 합쳐졌다.** 예전에는 ①이 `aimCue`(0번 줄),
   ②가 `crackCue`(1번 줄)였다. 길이 예산 때문에 두 줄을 「다시 조준해도 안 되더라고요.
   전제를 의심했죠.」 한 줄로 합쳤으므로 `cue(1)` 은 **없는 줄**을 가리킨다(엔진이 조용히
   0번으로 접혀 오작동한다). 그래서 **②를 같은 `c0` 의 뒤쪽으로 옮겼다** — `settle.js` 가
   같은 사유로 한 것과 같은 수술이다.
   🔑 자리를 고를 때 **낱말이 나오는 시각에 맞췄다 — 검수가 wav 로 실측한 값이다**
   (2026-08-08 델타 재검): 문장 사이 침묵이 1,980~2,160ms 라 「전제」 발성 시작 =
   **+2,160ms**, `c1 = clamp((c0 − 0.70)/0.30)` 에서 균열 알파가 서는 시각 = **+2,231ms**
   (Δ +71ms = 2프레임). 나레이션이 「전제」를 부르는 순간 판이 갈라진다.
   ⚠️ 초고 주석의 「+2.03초/+2.08초」는 옛 문구·산정 rel 기준이라 폐기.
   🔑 그래서 조준선 네 개도 앞으로 당겼다(`AIMS` 0.70 → 0.54, 수명 0.34 → 0.26).
   마지막 조준이 `c0 = 0.68` 에 끝나고 균열이 `0.72` 부터라 둘이 안 겹친다.
...
const AIMS = [0.06, 0.22, 0.38, 0.54];
...
export default {
  ...
  draw(root, { spec, t, cue }) {
    ...
    /* 🔴 예전엔 `cue(crackCue=1)` 이었다 — 자막 둘이 하나로 합쳐져 없는 줄이 됐다(위 주석).
       같은 c0 의 뒤쪽 30% 를 선형으로 다시 편다. **선형이어야 한다** — 아래 세 곳이
       `(c1 − a)/b` 를 다시 ease 하므로 여기서 미리 ease 하면 곡선이 두 번 먹는다. */
    const c1 = clamp((c0 - 0.70) / 0.30);
    ...
    if (upA > 0.02) {
      ...
      for (let i = 0; i < AIMS.length; i++) {
        const f = AIMS[i];
        const run = ease(clamp((c0 - f) / 0.14));
        if (run <= 0.002) continue;
        const alive = clamp((c0 - f) * 9) * clamp((f + 0.26 - c0) * 6);
