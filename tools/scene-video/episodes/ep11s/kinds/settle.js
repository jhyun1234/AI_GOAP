...
/* settle — 마주치면 그 자리에서 갚고, 못 만나도 그대로 남는다.
   ...
   ① meetCue 앞쪽 — 위 레인에서 두 표식이 다가와 붙고, 목수 위의 빚 캡슐이 의뢰인 위로
      ...
      끝난다**(c0 0.54 = 자막 시작 +1,864ms = 그 자막의 43.6%). 4단 구조가 된 뒤로는 이 샷
      뒤에 아웃트로 샷이 통째로 하나 더 있어 카드까지 여유가 크다 — **실측 5.74초**
      (페이오프 28.419초 · 카드 34.163초, 2026-08-08 검수 계측). ⚠️ 그 실측은 본문 조임
      **전**의 것이라 지금 값이 아니다. 조임 뒤 산정으로는 페이오프 27.0초 · 카드 34.3초라
      약 7.3초인데, **산정이지 실측이 아니다.** 재렌더 뒤 이 자리를 갈아 끼울 것.
      **앞쪽 절반에서 끝내는 설계는 그대로 둔다** — 카드 여유는 구조가 주는 것이고
      페이오프를 자막 뒤쪽에 두는 습관이 남으면 다음 회차에서 다시 먹힌다.
   ...
   ③ 같은 자막의 뒤쪽 — 두 레인 밑으로 강조색 밑줄이 왼쪽에서 오른쪽으로 그어진다.
      🔴 **2026-08-08 4단 구조 개정으로 근거가 바뀌었다.** 예전에는 이 밑줄이 **예고 자막**
      (S4[1], `holdCue`)에 물려 있었다 — 예고가 이 샷에 붙어 있었고, 그 구간이 아웃트로 카드에
      먹히기 직전 0.5초쯤이라 큰 면적의 변화가 필요했기 때문이다. 지금은 예고가 아웃트로
      샷(SO)으로 나갔고 **이 샷의 자막은 한 줄뿐**이라, `cue(1)` 은 존재하지 않는 줄을 가리킨다.
      그래서 같은 `c0` 의 뒤쪽 구간(0.66~1.0)으로 옮겼다. 페이오프(정산)는 c0 0.54 에서
      이미 끝나 있으므로 밑줄은 **그 뒤를 이어받아** 샷 끝까지 움직이는 것을 남긴다.
...
export default {
  ...
  draw(root, { spec, t, cue }) {
    ...
    /* 🔴 예전엔 cue(1)(예고 자막)이었다 — 그 줄이 아웃트로 샷으로 나가 이 샷은 한 줄뿐이다.
       같은 c0 의 뒤쪽으로 옮겨 페이오프(c0 0.54 완료) 뒤를 이어받게 했다. */
    const rule = ease(clamp((c0 - 0.66) / 0.34));
    ...
    /* ── 페이오프 뒤를 이어받는 변화 — 두 레인을 잇는 밑줄 ─
       🔴 이 샷에 예고 자막은 없다(아웃트로 샷 SO 로 나갔다). 옛 주석이 「예고 자막이 여는
       변화」였는데 그건 3단 구조 시절의 서술이다. 지금은 같은 자막(S4[0])의 뒤쪽 구간이다. */
    if (rule > 0.01) {
