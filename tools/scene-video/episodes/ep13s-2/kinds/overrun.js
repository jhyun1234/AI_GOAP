...
/* overrun — 뒤집힘과 남는 한 줄. 회차 축(「한 칸의 폭」)의 **한계** 배치다.
   ...
   가로 — 칩은 x 240 에서 들어와 150 에 앉는다(오른쪽 끝 최대 324 < w−22). 캔버스 밖으로
   안 나간다.
   🔴 실측 교정(마스터 판정): 흰 라벨 「최소 비용」의 오른쪽 끝은 22~84 가 아니라 **67.5** 이고,
   그 오른쪽 **최근접 강조색은 칩이 아니라 값 숫자(87.5)** 라 실제 최소 간격은 **20.0 CSS px** 다.
   라벨↔칩 간격도 66 이 아니라 **81.0px**. 접촉 0 이라 결함은 아니지만, **최근접 쌍을 잘못
   짚은 채 여유를 적으면 다음 사람이 그 쌍만 지킨다.** 겹침은 이름 댄 쌍이 아니라 **실측한
   최근접 쌍**으로 적을 것. */
