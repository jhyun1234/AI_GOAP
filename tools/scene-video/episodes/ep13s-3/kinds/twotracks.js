...
/* twotracks — 같은 시계가 두 갈래로 끝난다. 하루 만에 낫거나, 하루 반 만에 죽거나.
   ...
   ⏱ **자막 한 줄 안의 어순에 물렸다.** 이 샷의 자막은 한 줄
   (「하루 반이면 죽고, 간호받으면 하루에 낫거든요.」)이므로 `cue(0)` 하나를
   구간으로 갈라 쓴다 — 앞 절 「하루 반이면 죽고」에서 하단 행이 끝까지 차고 표식이
   꺼질 듯 서고(c0 0~0.36), 뒤 절 「간호받으면 하루에 낫거든요」에서 상단 행이 2/3 에서
   멎고 회복 표식이 뜬다(c0 0.40~0.70). 그 뒤 두 끝 사이에 반나절 브래킷이 걸린다(0.74~).
   🔴 옛 주석은 존재하지 않는 두 줄짜리 자막을 인용하고 있었다(초고에서 한 줄로 합친 뒤
   갱신을 빠뜨렸다). **주석이 대본을 인용할 때는 대본이 바뀔 때 같이 고쳐야 한다** —
   안 그러면 다음 사람이 주석을 믿고 cue 인덱스를 잘못 읽는다.
