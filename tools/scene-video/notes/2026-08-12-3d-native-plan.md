...
| 2차-2 엔진 캔버스 합성 | ✅ | `kinds/scene3d.js` · `engine.js` 대기 채널 · **`build/video.mp4` 48.4초** |
| 2차-3 게이트 갱신 | ✅ | 팔레트 3층 · 길이 59초 · 잘림 선언예외 + `test_stage.py` |
...
### 엔진 합성까지 끝났다 — `build/video.mp4` 가 완성본이다

블렌더 프레임이 **엔진 캔버스 안에서** 그려진다(설계 §7-1). 자막·HUD·챕터칩·아웃트로 카드가
그 위에 얹히고 소리가 붙는다. **게이트 33종 전부 통과**(경고 3건은 길이 권장대역).
...
- 🔴 **`.vis` 상자는 1080 이 아니라 970 이다.** 계획 문서가 「쇼츠 폭 그대로」로 적어 뒀는데,
  ...
  그래서 `blender3d/test_stage.py` 가 CSS 에서 읽어 대조한다 — 둘 중 하나만 고치면 걸린다.
- 🔴 **`kind` 이름은 구조다.** `check.mjs` 가 `kind === 'intro'` 로 인트로 유무·제목 이행
  시작점·4단 구조를 센다. 그래서 이름은 그대로 두고 다섯 파일이 `scene3d.js` 를 재수출한다.
- 🔴 **샷 진입 모션을 껐다**(`enter: "cut"`). 기본값 `up` 은 2D 도형판의 것이고, 3D 는
  카메라가 이음새를 진다 — 엔진이 또 밀어 넣으면 이어진 카메라가 샷마다 튄다.
- 🔴 **프레임이 안 왔는데 캡처되면 그 프레임만 빈다.** 클립 레이어(W5)가 밟았던 함정이라
  같은 장치를 일반화했다 — `engine.js` 의 `drawWaits`. kind 가 약속을 걸면 seek 이 돌려주고
  render/check 가 프레임마다 await 한다. **약속 안에서 다시 그린다**(기다리기만 하면 늦다).
  결정성 3패스 242프레임 불일치 0.

### 🔴 그다음 판정 — 「움직임이 기계적이고 단조롭다」 (2026-08-13)

판정: **「① 움직임이 너무 기계적이고 단조롭다. ② Anticipation 을 뚜렷하게, ③ 끝에
Overlapping Action 으로 관성을, ④ 전체 속도감은 Snappy 하게, ⑤ 관절 궤적을 Arc 로.」**

🔑 **이번에는 피사체가 아니라 곡선이 틀렸다.** 앞의 여덟 판은 「밋밋하다」를 연출로 듣고
카메라를 만지다 헛돌았지만, 이 판정은 *무엇을 그리는가* 가 아니라 *어떻게 움직이는가* 를
가리킨다 — 그래서 고칠 자리가 `motions.py` 하나였다. **여섯 샷이 전부 이 파일을 지난다.**

고친 것 (`blender3d/motions.py`, 손잡이 여덟 개가 파일 맨 위에 모여 있다):

| 원칙 | 장치 | 어디에 |
|---|---|---|
| Anticipation | `_snappy` 의 앞 구간이 **음수** — 반대로 물러났다 간다 | `look_up`(먼저 숙인다) · `reach`(팔을 먼저 뺀다) · `chop`(도끼를 앞에 떨군다) · `unison.march` 의 도는 비트 |
| Snappy | `_snap` (=`1-(1-u)^2.6`). `_ease` 는 대칭이라 시작이 굼뜨다 | 한 번뿐인 동작 전부 · `chop` 의 들기 |
| Overlapping | 말단이 몸통보다 늦는다(`LAG_MID/LAG_TIP`, 주기 동작은 `PH_MID/PH_TIP`) + 끝에서 오버슛했다 앉는다(`OVER`) | 전 동작. `hand.*` 뼈가 이때 처음 쓰였다 |
| Arc | Z(좌우)를 한 박자 어긋내 궤적을 평면 밖으로 부풀린다(`_arc`) | `walk` 팔·다리 · `reach` · `farm` · `chop` · `draw` |

- 🔴 **`thigh` 의 X 는 안 건드렸다.** 보폭이 거기서 나오고 `WALK_SPEED` 가 그 값을
  역산해 쓴다 — 손대면 발이 미끄러지거나 끌린다(`test_walk_speed_...` 가 대조한다).
  기계 티는 **나머지 뼈로만** 뺐다: 골반 비틀림 ↔ 척추 반대 비틀림, 팔이 다리보다 늦게,
  머리가 몸통과 반대로 돌아 시선 고정.
- 🔴 `chop` 은 셋으로 갈랐다: 예비로 앞에 떨궜다 → 올리고 → **가속하며** 내리치고 →
  바닥에서 되튄다. 예전 판은 들기·치기 둘 다 `_ease` 라 대칭이었고, 그래서 도끼가
  「떨어진다」가 아니라 「내려간다」로 읽혔다.
- 🔴 `motions.blend` 가 **끝점에서 원본을 그대로** 돌려준다. `p+(q-p)*1.0` 의 마지막 자리
  오차가 이음새에서 한 프레임짜리 떨림으로 나온다 — 겹치는 뼈가 늘면서 드러났다.
- 검사 여섯을 새로 박았다(`test_motions.py`): 예비 동작 · 오버슛과 정확한 정착 ·
  말단 지연 · Arc · `chop` 되튐. 🔑 `chop` 검사의 잣대를 `CHOP_LIFT` **상수에 매었다** —
  값을 박아 두면 비율을 고쳤을 때 검사가 딴 구간을 재게 된다.

### 🔴 그다음 판정 — 「걷게 해라 · 굳히지 마라 · 파란 보드를 걷어내라」 (2026-08-13)

판정 넷. 전부 **연출이 아니라 그 순간 사람이 뭘 하고 있는가**를 가리켰다.

| # | 어디 | 무엇 |
|---|---|---|
| ① | 본문1 ④⑤ | 「겨울에 잠깐 갈라졌다 합쳐졌어요」에서 **서로 다른 쪽으로 걸어** 흩어지고 **한 자리로 걸어** 돌아온다 |
| ② | 본문3 ⑦ | 「여섯이 드디어 다르게 움직였어요」에서 **걸어서** 흩어진다 |
| ③ | 아웃트로 | 파란 한기 띠를 걷어내고 **붉은 덩이**가 주민 앞에서 위아래로 흔들리며 다가온다 |
| ④ | 아웃트로 | 여섯을 **안 굳힌다.** 그냥 마을을 걸어 돌아다닌다 |

- 🔴 **셋 다 「자리만 밀던」 것을 「걷는 것」으로 바꿨다.** 앞 판은 하던 일(밭일·뻗기)을
  그대로 한 채 `location` 만 밀어서 **미끄러졌다** — 5비트 훅이 반려된 그 문제의 반대편이다.
  걸리는 시간은 전부 **거리 ÷ `motions.WALK_SPEED`** 로 역산한다(`SCATTER_SPAN`·`WALK_SPAN`).
- 🔴 **군무는 안 깼다.** 본문1 의 여섯은 흩어질 때도 *같은 걷기*(위상 오차 0)를 하고 **방향만**
  다르다. 본문3 은 *동작 종류*가 갈라진다 — 두 사건이 안 뭉개진다.
- 🔴 **`unison.SPLIT_FAN` 이 새로 생겼다.** 「무리 중심에서 바깥으로」 방사하던 예전 식은
  주민 0 을 집 둘 사이에 처박았다(궤적 −0.69 m). 마을 북쪽은 집 셋이 벽이라 **−40°~−140°**
  부채꼴(열린 카메라 쪽)로 보낸다. 본문3 과 아웃트로가 **같은 상수**를 읽는다.
- 🔴 아웃트로의 고리 반지름·도는 쪽은 **`ring_for` 가 소품 여유를 재서 고른다.** 손으로
  적으면 마을이 바뀌는 날 누가 집을 뚫는다. 지금 0.48 m(한 바퀴), 주민 사이 최소 0.76 m.
  🔑 **한 바퀴짜리**를 쓴다 — 큰 고리를 골랐더니 여섯 중 셋이 화면 밖으로 걸어 나갔다.
- 🔴 붉은 덩이는 **지붕(1.85) 위에서 시작해 내려온다.** 처음부터 머리 높이에 뒀더니
  멀리 있는 동안 「지붕 위의 빨간 상자」로 읽혔다. 겨냥도 무리를 따라가되 **승인된 판의
  치우침(마을 쪽 0.9 m)을 그대로 둔 채** 민다 — 한가운데로 옮겼더니 화면이 다시 잡혔다.
- 🔑 뜻층 예산: 아웃트로가 한기+앰버 → **붉은색+앰버**. 한기는 「겨울」(본문1)에 이미
  배정된 뜻이라, 같은 도형에 뜻 둘을 얹으면 본문1 의 겨울이 흐려진다.
- `motions.reach`/`freeze` 는 이제 이 회차가 안 쓴다. **지우지 않았다** — 짝이고
  `render_motion_sheet.py` 가 아직 보여 준다. 굳음을 되살릴 일이 있으면 그대로 있다.

### 🔴 그다음 판정 — 「자막은 숫자로 · 마지막을 다시」 (2026-08-13, 둘째)

판정 넷.

1. **자막의 숫자는 아라비아 숫자로.** 「서른」→`30`, 「백여든」→`180`, 「여섯이」→`6명이`.
2. **TTS 는 100 이상을 한자어로.** 「백여든」이 아니라 **「백팔십」**(백십·백이십·천백팔).
   🔑 규약을 [새-회차-runbook §3-①](./2026-08-13-새-회차-runbook.md) 에 표로 박았다 — 회차마다 다시 정하지 않게.
   🔴 `say` 를 고치면 **비트가 바뀐다.** 이 한 글자로 S3 비트가 1.499 → 1.509 초가 되어
   홀드 상한(1.5)을 넘겼다. 그 줄 `pauseAfter` 를 200 → 100 ms 로 줄여 되돌렸다(10.46초).
3. **아웃트로의 고리 걷기를 걷어냈다.** 여섯이 서로 다른 방향이어도 **같은 곡선을 같은
   속도로** 그리니 「셋씩 짝지어 원을 돈다」로 읽혔다 — 군무는 본문3 이 이미 깨뜨렸는데
   여기서 되살아나면 결말이 무효가 된다. 지금은 **동작 자체가 셋으로 갈린다**:
   밭일 하나 · 밥 둘 · 걷기 셋. 🔑 **누가 무엇을 하는지는 자리가 정한다** — 밭 위에 선
   사람이 밭을 갈고 불 곁에 선 둘이 밥을 먹는다(이름표를 손으로 붙이면 밭 밖에서 밭을 간다).
4. **붉은 덩이는 끝까지 허공에 있다**(z 1.37 아래로 안 내려온다). 땅에 가까워지면
   「굴러오는 상자」가 된다. 단언으로 박아 뒀다.

- **`motions.eat` 이 새로 생겼다**(주기 1.5초). 🔴 **두 손을 같이 움직이지 않는다** —
  받치는 손은 가만히 있고 한 손만 입으로 오간다. 둘 다 오르내리면 박수로 읽힌다.
- 🔴 **걷는 셋이 갈 쪽을 손으로 정하지 마라.** 세 번 해 봤고 세 번 다 틀렸다:
  바깥으로 곧장 → 프레임 밖(45mm 수평 화각 43.6°) · 무리 쪽으로 가로질러 → 서로를 뚫음
  (실측 0.12 m) · 카메라 쪽으로 → 한 명이 화면을 반쯤 채움. 지금은 `render_outro._score`
  가 **소품 여유 · 서로 간 거리 · 화면 가로(2.55 m) · 깊이(2.10 m)** 를 재서 고른다.
  🔑 프레임은 가로가 먼저 터진다 — 「무리에서 몇 m」 하나로 재면 가로와 깊이가 섞여 못 막는다.
  지금 소품 여유 1.31 m · 주민 최소거리 0.65 m.

### 🔴 다음 세션이 **가장 먼저** 읽을 것

**ep15s-1 은 끝났다.** 다음 편(ep15s-2~5)을 만든다면 이 문서가 아니라
👉 **[`2026-08-13-새-회차-runbook.md`](./2026-08-13-새-회차-runbook.md)** 로 가라.
회차 무관 파일 일곱 · 새로 쓰는 것 둘(대본·샷 스크립트) · **먼저 막아야 할 구멍 둘**
(인트로가 사실은 공유 안 됨 · `scene3d.js` 가 회차 폴더에 있음)이 거기 있다.

⚠️ ep15s-2~5 는 2D 판 `video.mp4` 가 **이미 있다.** 「만들기」가 아니라 「3D 로 다시 만들기」이고,
대본이 2D 도형을 전제하고 쓰여 있어 **대본 재작성이 먼저**다.

### 🔴 (ep15s-1 한정) 다음 세션이 **가장 먼저** 할 일

**사용자 판정을 받는다** — `episodes/ep15s-1/build/video.mp4`(완성본)를 보고, 처음 0.3 초에
