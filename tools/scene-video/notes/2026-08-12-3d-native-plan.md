...
| — 8비트 훅 | ✅ | `render_hook.py` → `3d/ep15s-1/hook8/hook8.mp4` |
...
**사용자 판정을 받는다** — `hook8.mp4` 를 보고, 자막 없이 처음 0.3 초에
...
- 🔴 **훅 길이 6.606 초는 못 늘린다.** SH 샷 내레이션 길이이고 엔진 타임라인이 그 값으로
  돈다(`build/timed.json`). 비트를 늘리면 길어지는 것이 아니라 **비트가 짧아진다** —
  여덟 비트면 비트당 0.826 초다. 늘리려면 대본부터 늘려야 한다.
