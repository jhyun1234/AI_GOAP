...
- **한다**: 다음 회차 선정 → 대본(씬 JSON) 작성 → 검수 → 승인 → 음성·렌더·점검 →
  붙여넣을 메타데이터 생성. **격일로 사람 없이 여기까지 간다**(대본은 클라우드가 격일,
...
- `scriptBy: "cloud"` (지금) — **클라우드 루틴이 격일 박자를 쥔다.** 마지막 `scene.json` 커밋으로부터
  1.5일이 지났는지를 **git 이력**으로 재고(파일 mtime 은 새 clone 에서 전부 체크아웃 시각이라 못 쓴다),
  `order` 안의 회차만 센다 — 손으로 만든 특별편이 본편 박자를 먹으면 안 된다.
  로컬은 간격 게이트를 **아예 보지 않는다**. 규칙이 하나뿐이다 — *아직 아무도 렌더 안 한 대본이 있으면 렌더한다.*
- `scriptBy: "local"` — 이 PC 가 대본까지 만들고, 간격 게이트도 여기서 본다.
...
🔴 **왜 격일이 아니라 매일인가.** 격일 박자는 이미 클라우드가 쥐고 있으므로 여기서 또 격일로 잠그면
