...
| | 에이전트 | 모델 | 산출 |
|---|---|---|---|
| 1 | `scene-planner` | sonnet | `.staging/01_planner_brief.md` |
| 2 | `scene-writer` | opus | `scenes/<ep>.json` + `.staging/02_writer_note.md` |
| 3 | `scene-reviewer` | opus | `.staging/03_review_report.md` (또는 작성팀에 반려) |
| 4 | `scene-master` | opus | `.staging/04_verdict.md` |
...
반드시 `tools/scene-video/.staging/` 에 파일로 남기고 다음 단계에 경로를 알려준다.
...
```
음성(캐시 재사용) → mp4 렌더 → `check.mjs` 점검 → `build/<ep>.upload.txt` 생성.
...
멈출 때는 `.staging/` 에 사유를 남긴다. **애매하면 통과시키지 말고 멈춘다** —
영상은 글과 달리 되돌리는 비용이 크다.

## 사람이 이어받는 지점

1. `build/<ep>.mp4` 를 스튜디오(<https://studio.youtube.com>)에 끌어다 놓는다
2. `build/<ep>.upload.txt` 의 제목·설명·태그를 붙여넣는다
3. 공개로 전환한다
